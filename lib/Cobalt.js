...
const Cobalt = async (input, mode = 'search', options = {}) => {
    ...
    if (mode === 'search') {
        ...
        const audioData = await yts({ videoId });
        if (!audioData) throw new Error('Audio nya gak ada btw 😊');
        
        const download = { ...videoData, audioData, mediaUrl: await request(videoId, mode, options.quality, options.format) };
